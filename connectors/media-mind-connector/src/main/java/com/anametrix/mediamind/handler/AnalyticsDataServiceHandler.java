...
public class AnalyticsDataServiceHandler{
	...
	private APIDateTime convertTOAPIDate(Date date){
		...
		apiDate.setHour(calendar.get(Calendar.HOUR_OF_DAY));
		apiDate.setMinute(calendar.get(Calendar.MINUTE));
		apiDate.setSecond(calendar.get(Calendar.SECOND));
		return apiDate;
	}
	

	public Object getReport(Date reportStartDate,Date reportEndDate,int campaingId,int cookieWindowClicks,int cookieWindowImpressions,
			int conversionAttributionModel,int dataLevelID, Class clz) throws RemoteException  {
